...
class TicketFactory {
    ...
    async submit(signedTicket) {
        ...
        const counterPartySecret = hopr_utils_1.u8aXOR(false, ticket.challenge, secret);
        const pre_image = await this.channel.coreConnector.hashedSecret
            .getPreimage(ticket.onChainSecret)
            .then((res) => res.preImage);
        // challange = hash(secret A + secret B)
        const transaction = await signTransaction(hoprChannels.methods.redeemTicket(hopr_utils_1.u8aToHex(pre_image), hopr_utils_1.u8aToHex(ticket.channelId), hopr_utils_1.u8aToHex(counterPartySecret), // secret A
        hopr_utils_1.u8aToHex(secret), // secret B
        ticket.amount.toString(), hopr_utils_1.u8aToHex(ticket.winProb), hopr_utils_1.u8aToHex(r), hopr_utils_1.u8aToHex(s), v), {
